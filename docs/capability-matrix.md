...
| Pattern color space (tiling and shading patterns) | Phase 5 | Not started | `cs`/`CS Pattern` is recognized and recorded (`internal/content/colorspace.go`), but `scn`/`SCN` with a pattern name operand is still explicitly rejected as unsupported - tiling/shading pattern rendering itself remains further Phase 5 work. |
