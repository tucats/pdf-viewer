...
| `encrypted.pdf` | Structurally identical to `minimal-blank-page.pdf`, but its trailer declares `/Encrypt 5 0 R`, pointing at a placeholder Standard-security-handler dictionary (not a byte-accurate one - see `buildEncrypted`'s doc comment). The Phase 6 regression fixture for this project's "password handling" decision: `Open` must reject any file whose trailer names `/Encrypt`, with a clear error wrapping `ErrEncrypted`, at open time rather than failing confusingly later. |
