...
// fakeResolver is a minimal in-memory Resolver for tests: objects maps an
// object number to the value Resolve should return for it (matching
// internal/parser.Document.Resolve's contract that an unknown number
// resolves to Null rather than an error), and DecodeStream applies
// internal/filter.Decode directly rather than needing a real parsed
// document - this is the same small test double every other internal
// package in this module defines its own copy of (see, for example,
